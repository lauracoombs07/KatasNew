...
describe('A simple iterable without items inside, implementing the right protocol', () => {
    ...
    }//make an ES5 function and return an empty object {} return Object{} done:true
    ...
    beforeEach(function() {
      iterable = {
        [Symbol.iterator]: iteratorFunction//Key value [Symbol.iterator] in square brackets cause it needs to compute something. : iteratorFunction
      };//remove () from iteratorFunction
    });
    ...
    describe('using the iterable', function() {
      it('it contains no values', function() {
        let values = '';// set values to = an '' so that value of values = ''
      ...
      it('has no `.length` property', function() {
        const hasLengthProperty = false;//change iterator to false.
        assert.equal(hasLengthProperty, false);
      });
      describe('can be converted to an array', function() {
        it('using `Array.from()`', function() {
          const arr = Array.from(iterable);//you can make an array from an iterable with Array.from
          assert.equal(Array.isArray(arr), true);
        });
        it('where `.length` is still 0', function() {
          const arr = Array.from (iterable);//Array.from (iterable)
