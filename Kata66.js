...
describe('An object literal can also contain getters', () => {
    it('just prefix the property with `get` (and make it a function)', function() {
      const obj = {
        get x() { return 'ax'; }// get goes before the property
      };
      assert.equal(obj.x, 'ax');
    });
    it('must have NO parameters', function() {
      const obj = {
        get x() { return 'ax'; }//() with nothing inside. No params. creating a func using get instead of writing function.
      };
      assert.equal(obj.x, 'ax');
    });
    it('can be a computed property (an expression enclosed in `[]`)', function() {
      const keyName = 'x';
      const obj = {
        get [keyName]() { return 'ax'; }//[ ] around keyName makes it a computed property
      };
      assert.equal(obj.x, 'ax');
    });
    it('can be removed using delete', function() {
      ...
      delete obj.x;//remove get function by deleting obj.x
