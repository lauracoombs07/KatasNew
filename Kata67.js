...
describe('An object literal can also contain setters', () => {
    describe('defining: a setter', function() {
      it('by prefixing the property with `set` (and make it a function)', function() {
        let theX = null;
        const obj = {
          set x(newX) { theX = newX; }//prefix with set. it CAN have a parameter
        };
        obj.x = 'the new X';
      ...
      it('must have exactly one parameter', function() {
        ...
        const obj = {
          set x(new2) { // <<<<=== it's not a setter yet!
            if (arguments.length === 1) {//set and add a param
      ...
      it('can be a computed property (an expression enclosed in `[]`)', function() {
        ...
        const obj = {
          ...
          set [publicPropertyName](bacon) { 
            this[privatePropertyName]=bacon }//compute pubProp return param
          // write the complete setter to make the assert below pass :)
        };//this refers to current scope.Give new value(bacon) ob
        obj.x = 'axe';
