// 32: array - `Array.prototype.find` 
// To do: make all tests pass, leave the assert lines unchanged!
// Follow the hints of the failure messages!

describe('`Array.prototype.find` makes finding items in arrays easier', () => {
  it('takes a compare function', function() {
    const found = [true].find(boolean => true);//give it a variable. variable =>true
    assert.equal(found, true);//returns the first value found
  });
  it('returns the first value found', function() {
    const found = [0, 1, 2, 3].find(item => item > 1); // added 2 to the array
    assert.equal(found, 2);
  });
  it('returns `undefined` when nothing was found', function() {
    const found = [1, 3].find(item => item === 2);//by removing the 2, it cannot find 2 and becomes undefined
    assert.equal(found, void 0);
  });
  it('combined with destructuring complex compares become short', function() {
    const bob = {name: 'Bob'};
    const alice = {name: 'Alice'};
    const found = [bob, alice].find(({name}) => name === 'Alice');
    assert.equal(found, alice);//by defining the deep equal to be 'Alice', It skips over anything that is not 'alice'
  });
});
