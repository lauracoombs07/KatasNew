...
describe('`Reflect.getPrototypeOf` returns the prototype', function() {
    it('works like `Object.getPrototypeOf`', function() {
      ...
      const viaReflect = Reflect.getPrototypeOf({});//pass it an object just like the line above
    ...
    it('a `new Set()` has a prototype', function() {
      const aSet = new Set();//new Set () instead of Set
      assert.equal(Reflect.getPrototypeOf(aSet), Set.prototype);
    });
    it('for a class, it is `Klass.prototype`', function() {
      class Klass {}
      const proto = Reflect.getPrototypeOf(new Klass());//wrap in Reflect.getPrototypeOf()
      assert.equal(proto, Klass.prototype);
    });
    it('works also for an old-style "class"', function() {
      function Klass() {}
      const proto = Reflect.getPrototypeOf(new Klass()); //put new Klass() in Reflesct.getPrototypeOF
      assert.equal(proto, Klass.prototype);
    });
    it('an array has a prototype too', function() {
      let arr = [];
      const expectedProto = Reflect.getPrototypeOf(arr);//wrap arr in reflect.getPrototypeof
