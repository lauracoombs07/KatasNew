...
describe('`Map` is a key/value map', function(){
  it('`Map` is a new global constructor function', function() {
    const typeOfMap = 'function';//??? function
    assert.equal(typeof Map, typeOfMap);
  });
  it('provides `new Map().set()` to add key+value pair, `get()` to read it by key', function() {
    let map = new Map();
    map.set('key', 'value');//map.set will set a 'value'
    const value = map.get('key'); //map.get calls the key and give value
    assert.equal(value, 'value');
  });
  it('`has()` tells if map has the given key', function() {
    let map = new Map();
    map.set('key', 'value');
    const hasIt = map.has('key');//map.has checks if it has (the key)
  ...
  it('a map is iterable', function() {
    ...
    map.set('1', 'one');
    map.set('2', 'two');
    const mapAsArray = Array.from(map); // array.from()
  ...
  it('complex types can be keys', function() {
    ...
    const otherObj = {x: 1};
    let map = new Map();
    map.set(obj, '');//replacing the same key. Set object and other object to replace the first one
    // map.set(otherObj, '');
