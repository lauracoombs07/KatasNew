...
describe('initialize a `Map`', function(){
    it('a `new Map()` is empty, has size=0', function() {
      const mapSize = new Map().size;//accessing the property size of map and comparing it to the size of the new map
      assert.equal(mapSize, 0);
    });
    it('init Map with `[[]]` has a size=1', function() {
      const mapSize = new Map([[]]).size;//a single array and no size. a nested array has one array inside so size of 1
      assert.equal(mapSize, 1);
    });
    it('init a Map with `[[1]]` is the same as `map.set(1, void 0)`', function() {
      let map1 = new Map([[1]]);//value of one. initialize a map with value or values in it
    ...
    it('init Map with multiple key+value pairs', function() {
      const pair1 = [1, 'one'];
      const pair2 = [2, 'two'];
      const map = new Map([pair1, pair2]);//map requires square brackets to group the objects that are going inside of it
    ...
    it('keys are unique, the last one is used', function() {
      ...
      const pair3 = [1, 'eins'];
      const pair4 = [2, 'two'];
      const map = new Map([pair2, pair1, pair3, pair4]);//using the last 3 changed pair 2 to pair 3
    ...
    it('init Map from an Object, is a bit of work', function() {
      ...
      keys.forEach(key => map.set(key, obj[key]));//need to get the value indirectly through object keys
