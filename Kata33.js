...
describe('`Array.prototype.findIndex` makes finding items in arrays easier', () => {
  it('takes a compare function, returns the index where it returned true', function() {
    const foundAt = [false, true].findIndex(i => i === true);//inline function using i for index triple = true
    assert.equal(foundAt, 1);
  });
  it('returns the first position it was found at', function() {
    const foundAt = [0, 1, 1, 1].findIndex(item => item === 1);//made = a triple=, when you find it, stop looking
    assert.equal(foundAt, 1);
  });
  ...
  it('the findIndex callback gets the item, index and array as arguments', function() {
    ...
    function theSecondThree(item, index, arr) {//add item to the parameter(item, index, arr)
      const preceedingItems = arr.slice(0, index);//showing us that you need 3 callbacks
  ...
  it('combined with destructuring complex compares become short', function() {
    ...
    const foundAt = [bob, alice].findIndex(({name:{length:l}}) => l > 3);//used variable l in the inline function instead of length
    assert.equal(foundAt, 1);
  });
});
