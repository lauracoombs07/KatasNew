...
describe('`Symbol.for` for registering Symbols globally', function() {
  it('creates a new symbol (check via `typeof`)', function() {
    const symbolType = typeof Symbol.for('symbol name');//typeof checks the type of whatever comes next.The symbol is an object.
  ...
  it('stores the symbol in a runtime-wide registry and retrieves it from there', function() {
    ...
    const sym1 = Symbol.for('new symbol');//when second one was new symbol1 they were not equal
  ...
  describe('`.toString()` on a Symbol', function() {
    it('also contains the key given to `Symbol.for()`', function() {
      const description = Symbol('new symbol').toString();//if you turn a symbol into a string, it LITERALLY turns the symbol into a string
