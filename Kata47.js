// 47: Set - basics
// To do: make all tests pass, leave the assert lines unchanged!
// Follow the hints of the failure messages!

describe('`Set` lets you store unique values of any type', function(){
    it('`Set` is a new global constructor function', function() {
      const typeOfSet = 'function';//looking for ___ function
      assert.equal(typeof Set, typeOfSet);
    });
    it('every value in a set is unique', function() {
      let set = new Set();
      set.add(1);
      set.add(2);//change the second value. they can't be the same value
      const expectedSize = 2;
      assert.equal(set.size, expectedSize);
    });
    it('the string "1" is different to the number 1', function() {
      let set = new Set('1');//put 1 in quotes when you create a new set you can give it parameters
      set.add(1);
      set.add('1')//'1' is different that 1
      assert.equal(set.size, 2);
    });
    it('even NaN is equal to NaN', function() {
      let set = new Set();
      set.add(NaN);
      set.add(NaN);//set regards NaN as identical
      assert.equal(set.size, 1);
    });
    it('+0 and -0 are seen as equal', function() {
      let set = new Set();
      set.add(+0);
      set.add(0);
      set.add(-0);//remove quotes to make - + and 0 are the exact same
      assert.deepEqual([...set.values()], [+0]);
    });
  });
  