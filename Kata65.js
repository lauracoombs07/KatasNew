...
describe('`Set` API overview', function(){
    ...
    it('a Set can be created from an array', function() {
      let set = new Set(api);//api is an array doesn't need to be called with []
      assert.deepEqual(Array.from(set), api);
    });
    it('`size` is the number of values', function() {
      const theSize = set.size;//no property called count. use .size
    ...
    describe('returns an iterator that contains all values', function() {
      ...
      it('`keys()`', function() {
        const allKeys = set.keys();//set.keys not Object.keys(key)
        assert.deepEqual([...allKeys], api);
      });
      it('`values()`', function() {
        const allValues = set.values();//change value to values
        assert.deepEqual([...allValues], api);
      });
      it('`[Symbol.iterator]()`', function() {
        const iteratorKey = Symbol.iterator;//fill in the blank to Symbol.iterator
        assert.deepEqual([...set[iteratorKey]()], api);//assert is spreading contents of the set
      });
    });
  });
