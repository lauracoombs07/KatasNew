...
describe('Arrow functions', function() {
  it('are shorter to write, instead of `function(){}` write `() => {}`', function() {
    var func = '() => {}';//write an arrow function and place in single quotes
    assert.equal('' + func, '() => {}');
  });
  ...
  });//calling 2 parameters. must be in parens. also put both params in the parens
  it('the function body needs parens to return an object', () => {
    var func = () => ({iAm: 'an object'});
    assert.deepEqual(func(), {iAm: 'an object'});
  });
});//to return the object, it must be surrounded in parens
